...
<div id="content">
  ...
  $db = $data->getDbCon();
  $q="
  SELECT Players.PlayerName Player, COUNT(DISTINCT PlayerStats.GameID) Matches, ROUND(SUM(Playtime)/3600, 1) Hours, ROUND(SUM(Kills)/COUNT(DISTINCT PlayerStats.GameID), 2) AvgKills, ROUND(AVG(Damage)/AVG(Playtime)*60,2) DPM, ROUND(AVG(Heals)/AVG(Playtime)*60,2) HealsPM
