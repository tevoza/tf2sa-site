...
SELECT 
  ...
  ROUND(SUM(Assists)/COUNT(DISTINCT ps.GameID), 1) Assists, ROUND(SUM(Damage)/SUM(Playtime)*60,1) DPM, 
  ROUND(SUM(DamageTaken)/SUM(Playtime)*60,1) DTM, ROUND(SUM(Heals)/SUM(Playtime)*60,1) HPM
FROM 
  PlayerStats ps, Players p, Games g, ClassStats cs
WHERE 
  ps.SteamID=p.SteamID AND ps.GameID=g.GameID and ps.PlayerStatsID=cs.PlayerStatsID AND Date > {$cutoff} AND ClassID IN (7)
GROUP BY ps.SteamID
...
FROM 
  PlayerStats ps, Players p, Games g, ClassStats cs
WHERE 
  ps.SteamID=p.SteamID AND ps.GameID=g.GameID and ps.PlayerStatsID=cs.PlayerStatsID AND ClassID IN (7)
GROUP BY ps.SteamID
