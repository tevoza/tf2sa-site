...
<div id="content-wrap">
<?php $sidebar->printSideBar('soldier')?>
<div id="content">
  <?php
  $data = new dataAccess();
...
WHERE 
  ps.SteamID=p.SteamID AND ps.GameID=g.GameID and ps.PlayerStatsID=cs.PlayerStatsID AND Date > {$cutoff} AND ClassID IN (2)
GROUP BY ps.SteamID
...
FROM 
  PlayerStats ps, Players p, Games g, ClassStats cs
WHERE 
  ps.SteamID=p.SteamID AND ps.GameID=g.GameID and ps.PlayerStatsID=cs.PlayerStatsID AND ClassID IN (2)
GROUP BY ps.SteamID
