...
<div id="content">
  ...
  $db = $data->getDbCon();
  $q="
  SELECT Players.PlayerName Player, COUNT(DISTINCT PlayerStats.GameID) Matches, ROUND(SUM(Playtime)/3600, 1) Hours, SUM(Kills) Kills, ROUND(SUM(Kills)/COUNT(DISTINCT PlayerStats.GameID), 2) AvgKills, ROUND(AVG(Damage)/AVG(Playtime)*60,2) DPM
  FROM PlayerStats, Players, Games
  WHERE PlayerStats.SteamID=Players.SteamID AND Games.GameID=PlayerStats.GameID AND ClassID = 1 AND Games.Date > ".$cutoff."
